 ...
 * two live product cards with colour swatches + price, then a time-sensitive
 * countdown band. Content is FITZO / live catalogue.
 */
...
function ProductCard({ product }: { product: ConceptProduct }) {
  return (
    <div className="group flex flex-col">
      <Link href="/products" className="relative block aspect-[3/4] overflow-hidden rounded-[14px] bg-[#f0eeeb]">
        {product.image ? (
          <Image
            src={product.image}
            alt={product.title}
      ...
      <div className="mt-4 text-center">
        ...
        <div className="mt-3 flex items-center justify-center gap-2">
          {SWATCHES.map((c, i) => (
            <span
              key={c}
              className={`h-4 w-4 rounded-full ring-1 ring-[#d8d3cb] ${i === 0 ? "ring-2 ring-[#1a1a1a]" : ""}`}
...
export function FeaturedProduct({
  ...
  // 6 days, 11h out — matches the "678 days / 11 / 30 / 56" vibe loosely.
    ...
    <section className="bg-white pb-4 pt-10 sm:pt-14">
      <div className="mx-auto max-w-[1400px] px-5 lg:px-8">
        <div className="cx-rise text-center">
          ...
          <h2 className="mt-3 font-sans text-[clamp(2rem,4vw,3rem)] font-black uppercase text-[#1a1a1a]">
            Featured Product
          </h2>
        </div>

        <div className="mt-12 grid gap-6 lg:grid-cols-[1.5fr_1fr_1fr]">
          {/* Promo image card */}
          <Link href="/products" className="group relative block overflow-hidden rounded-[16px] bg-[#f0eeeb]">
            ...
            <div className="absolute bottom-8 left-8 right-8 text-white">
              ...
              <p className="mt-3 max-w-[300px] text-[14px] text-white/85">
                Explore our curated edit of clean, wear-anywhere staples.
              </p>
              <span className="mt-6 inline-flex h-11 items-center border border-white/70 px-7 text-[11px] font-semibold uppercase tracking-[0.18em] transition group-hover:bg-white group-hover:text-[#1a1a1a]">
                Shop now
              </span>
            </div>
          </Link>
        ...
        {/* Countdown band */}
        <div className="mt-6 flex flex-col items-center justify-between gap-6 rounded-[16px] bg-[#faf6f1] px-8 py-8 sm:flex-row">
          <div>
            <h3 className="font-sans text-[26px] font-bold leading-tight text-[#1a1a1a]">
              Summer sale — time-sensitive deals await!
            </h3>
            <p className="mt-1 text-[14px] text-[#6b6b6b]">Book a try-on slot before the window closes.</p>
          </div>
          <div className="flex items-center gap-4">
            {([["Days", t.days], ["Hours", t.hours], ["Mins", t.mins], ["Secs", t.secs]] as const).map(([label, val]) => (
              <div key={label} className="text-center">
                <div className="grid h-16 w-16 place-items-center rounded-xl bg-white font-sans text-[28px] font-bold tabular-nums text-[#1a1a1a] shadow-sm">
                  {val}
                </div>
                <p className="mt-2 text-[10px] font-semibold uppercase tracking-[0.14em] text-[#8a8a8a]">{label}</p>
              </div>
            ))}
            <Link href="/products?sale=true" className="ml-2 hidden h-12 items-center bg-[#b0703f] px-7 text-[12px] font-semibold uppercase tracking-[0.16em] text-white transition hover:bg-[#98602f] sm:inline-flex">
              Shop now
            </Link>
          </div>
        </div>
