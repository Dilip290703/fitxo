      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="space-y-4">
          <div className="bg-white border border-line rounded-xl p-5">
            <h3 className="text-xs font-semibold text-soft uppercase tracking-wide mb-3">Customer</h3>
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                <p className="text-sm font-medium text-ink">{order.users?.name ?? '—'}</p>
                <p className="text-xs text-muted">{order.users?.email}</p>
              </div>
            </div>
            <p className="text-xs text-soft">{order.users?.phone ?? '—'}</p>
            <Link href={`/admin/customers/${order.users?.id}`} className="text-xs text-info hover:text-ink mt-2 inline-block">
              View profile →
            </Link>
          </div>

          <div className="bg-white border border-line rounded-xl p-5">
            <h3 className="text-xs font-semibold text-soft uppercase tracking-wide mb-3">Delivery Address</h3>
            {order.addresses ? (
          ...
          {/* Payment */}
          <div className="bg-white border border-line rounded-xl p-5">
            <h3 className="text-xs font-semibold text-soft uppercase tracking-wide mb-3">Payment</h3>
            <div className="space-y-2 text-sm">
              ...
              {order.razorpay_payment_id && (
                <div className="flex justify-between">
                  <span className="text-soft">Razorpay ID</span>
                  <span className="text-body font-mono text-xs">{order.razorpay_payment_id}</span>
                </div>
              )}
        ...
        <div className="lg:col-span-2 space-y-4">
          {/* Try window */}
          {tryDeadline && (
            <div className={`rounded-xl px-5 py-4 border ${minutesLeft !== null && minutesLeft < 5 ? 'bg-danger-bg border-danger-line' : 'bg-warn-bg border-warn-accent/50'}`}>
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-soft">
                    ...
                    {minutesLeft !== null && (
                      <span className={`ml-2 font-medium ${minutesLeft < 5 ? 'text-danger' : 'text-warn'}`}>
          ...
          <div className="bg-white border border-line rounded-xl p-5">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-xs font-semibold text-soft uppercase tracking-wide">Order Items</h3>
              {items.length > 0 && (
                <span
            ...
            <div className="space-y-3">
              {order.order_items?.map((item: {
                ...
                <div key={item.id} className="flex items-start gap-3 pb-3 border-b border-hairline last:border-0">
                  <div className="w-14 h-14 bg-sand rounded-lg overflow-hidden flex-shrink-0">
                    {item.image_url ? (
          ...
          {order.deliveries && order.deliveries.length > 0 && (
            <div className="bg-white border border-line rounded-xl p-5">
              <h3 className="text-xs font-semibold text-soft uppercase tracking-wide mb-3">Delivery</h3>
              {order.deliveries.map((delivery: {
                ...
                <div key={delivery.id} className="space-y-2">
                  ...
                  {delivery.riders?.users && (
                    <div>
                      <p className="text-xs text-muted">Rider: <span className="text-body">{delivery.riders.users.name}</span></p>
                      <p className="text-xs text-muted">{delivery.riders.users.phone}</p>
                    </div>
                  )}
          ...
          {order.notes && (
            <div className="bg-white border border-line rounded-xl p-5">
              <h3 className="text-xs font-semibold text-soft uppercase tracking-wide mb-2">Customer Notes</h3>
