  ...
  return (
    <div className="max-w-5xl space-y-6">
      <div className="flex items-center gap-3">
        <Link href="/admin/stores" className="text-sm text-gray-500 hover:text-white">← Stores</Link>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="lg:col-span-1">
          ...
          <div className="bg-gray-800 border border-gray-700 rounded-xl p-5 mt-4">
            <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-3">Store Managers</h3>
            {managers && managers.length > 0 ? (
              <div className="space-y-2">
                {managers.map((m) => (
                  <div key={m.id} className="flex items-center justify-between">
                    <div>
                      <p className="text-sm text-white">{(m.users as {name:string; email:string})?.name}</p>
                      <p className="text-xs text-gray-500">{(m.users as {name:string; email:string})?.email}</p>
                    </div>
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="bg-gray-800 border border-gray-700 rounded-xl p-5">
            <div className="flex items-center justify-between mb-3">
              ...
              <Link href={`/admin/inventory?store=${id}`} className="text-xs text-indigo-400 hover:text-indigo-300">View all →</Link>
            </div>
                ...
                <tbody>
                  {products.map((p) => (
                    <tr key={p.id} className="border-b border-gray-700/50">
                      ...
                      <td className="py-2 text-right text-gray-400">₹{p.base_price}</td>
                ...
                <tbody>
                  {orders.map((o) => (
                    <tr key={o.id} className="border-b border-gray-700/50">
                      <td className="py-2">
                        <Link href={`/admin/orders/${o.id}`} className="text-indigo-400 hover:text-indigo-300 font-mono text-xs">{o.order_number}</Link>
                      </td>
                      <td className="py-2"><StatusBadge status={o.status} size="sm" /></td>
                      <td className="py-2 text-right text-gray-400">₹{o.final_amount}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
