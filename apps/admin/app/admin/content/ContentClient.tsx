...
export default function ContentClient({ blocks }: { blocks: ContentRow[] }) {
  ...
  return (
    <div className="space-y-4">
      <button onClick={openNew} className="px-4 py-2 text-sm bg-indigo-600 hover:bg-indigo-500 text-white font-medium rounded-lg">
        + New content block
      </button>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-700 bg-gray-800/50">
              <th className="px-4 py-3 text-left text-xs font-semibold text-gray-400 uppercase tracking-wide">Key</th>
              ...
              <th className="px-4 py-3 text-left text-xs font-semibold text-gray-400 uppercase tracking-wide">Published</th>
              <th className="px-4 py-3 text-right text-xs font-semibold text-gray-400 uppercase tracking-wide">Action</th>
            </tr>
          </thead>
          <tbody>
            {blocks.length === 0 ? (
              <tr><td colSpan={5} className="px-4 py-12 text-center text-gray-500">No content blocks yet.</td></tr>
            ) : (
              blocks.map((b) => (
                <tr key={b.id} className="border-b border-gray-700/50 hover:bg-gray-800/50 transition-colors">
                  <td className="px-4 py-3 font-mono text-xs text-indigo-400">{b.key}</td>
                  <td className="px-4 py-3 text-sm text-white">{b.title}</td>
                  <td className="px-4 py-3"><StatusBadge status={b.type} size="sm" /></td>
                  <td className="px-4 py-3">
                    <span className={`text-xs ${b.is_published ? 'text-green-400' : 'text-gray-500'}`}>{b.is_published ? 'Published' : 'Draft'}</span>
                  </td>
                  <td className="px-4 py-3 text-right space-x-3">
                    <button onClick={() => openEdit(b)} className="text-xs text-indigo-400 hover:text-indigo-300 font-medium">Edit</button>
                    <button onClick={() => remove(b)} className="text-xs text-gray-500 hover:text-red-400">Delete</button>
                  </td>
                </tr>
              ))
            )}
      ...
      {form && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={() => !isPending && setForm(null)}>
          <div className="w-full max-w-lg bg-gray-800 border border-gray-700 rounded-xl p-6 space-y-4" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-sm font-semibold text-white">{form.id ? 'Edit' : 'New'} content block</h3>
            ...
            <div className="flex justify-end gap-2">
              <button onClick={() => setForm(null)} disabled={isPending} className="px-4 py-2 text-sm border border-gray-600 text-gray-300 rounded-lg hover:border-gray-500">Cancel</button>
              <button onClick={save} disabled={isPending || !form.key.trim() || !form.title.trim()} className="px-4 py-2 text-sm bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white font-medium rounded-lg">
