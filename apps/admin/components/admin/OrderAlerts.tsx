    ...
    <>
      ...
      <div className="fixed right-4 top-4 z-[60]">
        ...
        {bellOpen ? (
          <div className="absolute right-0 mt-2 w-[300px] overflow-hidden rounded-xl border border-gray-700 bg-gray-800 shadow-2xl">
            ...
            {alerts.length === 0 ? (
              <p className="px-4 py-6 text-center text-[13px] text-gray-500">No new orders yet.</p>
            ) : (
                ...
                {alerts.map((a) => (
                  <li key={`${a.orderId}-${a.at}`}>
                    <button
                      ...
                      className="flex w-full items-center justify-between gap-3 border-b border-gray-700/60 px-4 py-3 text-left last:border-0 hover:bg-gray-700/30"
                    >
                      <div className="min-w-0">
                        <p className="truncate font-mono text-[12px] font-semibold text-indigo-300">
                          {a.orderNumber}
                        </p>
                        <p className="text-[11px] text-gray-500">{timeAgo(a.at)}</p>
                      </div>
                      <span className="shrink-0 text-[12px] font-semibold text-gray-200">
      ...
      <div className="fixed bottom-4 right-4 z-[55] flex w-[300px] flex-col gap-3">
        {activeAlerts.map((a) => (
          <div
            key={a.orderId}
            className="overflow-hidden rounded-xl border border-indigo-500/40 bg-gray-800 shadow-2xl"
          >
            <div className="flex items-start justify-between gap-2 bg-indigo-600 px-4 py-2">
            ...
            <div className="px-4 py-3">
              <p className="font-mono text-[13px] font-semibold text-white">{a.orderNumber}</p>
              <p className="mt-0.5 text-[12px] text-gray-400">{formatCurrency(a.amount)}</p>
              <button
                type="button"
                onClick={() => openOrder(a.orderId)}
                className="mt-3 w-full rounded-lg bg-indigo-600 py-2 text-[11px] font-semibold uppercase tracking-[0.14em] text-white transition hover:bg-indigo-500"
              >
                View order →
              </button>
            </div>
