...
export default function ColorVariantBuilder({ value, onChange }: ColorVariantBuilderProps) {
  ...
  return (
    <div className="space-y-3">
      {value.map((color) => (
        <div key={color.id} className="border border-gray-700 rounded-xl overflow-hidden">
          {/* Color header */}
          <div
            className="flex items-center gap-3 px-4 py-3 bg-gray-800/60 cursor-pointer"
          ...
          >
            ...
            <span className="text-xs text-gray-500">{color.variants.length} sizes</span>
            <button
            ...
            <span className="text-gray-500 text-xs">{expanded === color.id ? '▲' : '▼'}</span>
          </div>

          {expanded === color.id && (
            <div className="px-4 py-4 space-y-4 bg-gray-900/30">
              {/* Color inputs */}
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <input
                      ...
                      value={color.color_hex}
                      onChange={(e) => updateColor(color.id, { color_hex: e.target.value })}
                      className="w-24 bg-gray-800 border border-gray-600 rounded-lg px-2 py-2 text-sm text-white font-mono"
                    />
                  </div>
                </div>
              </div>
              ...
              {/* Variants */}
              <div>
                <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-2">Sizes & Stock</p>
                {color.variants.length === 0 && (
                  <p className="text-xs text-gray-600 italic mb-2">No sizes added yet</p>
                )}
                  ...
                  {color.variants.map((v) => (
                    <div key={v.id} className="flex gap-2 items-center">
                      <input
                        ...
                        className="w-24 bg-gray-800 border border-gray-600 rounded-lg px-2 py-1.5 text-sm text-white"
                      />
                      <input
                        type="text"
                        value={v.sku}
                        ...
                        placeholder="SKU"
                        className="flex-1 bg-gray-800 border border-gray-600 rounded-lg px-2 py-1.5 text-sm text-white font-mono"
                      />
                      <input
                        type="number"
                        value={v.stock_qty}
                        ...
                        className="w-20 bg-gray-800 border border-gray-600 rounded-lg px-2 py-1.5 text-sm text-white"
                      />
                      <button
      ...
      <button
        onClick={addColor}
        className="w-full py-2.5 border border-dashed border-gray-600 rounded-xl text-sm text-gray-400 hover:text-white hover:border-gray-500 transition-colors"
      >
        + Add Color
      </button>
    </div>
