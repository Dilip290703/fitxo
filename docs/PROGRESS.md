...
- [~] **Live new-order alerts (Store + Admin)** — Supabase Realtime pop-up + bell + chime when a new order is placed; no polling. Store subscribes to `order_items` INSERT (RLS-scoped, deduped per order), Admin to `orders` INSERT. Needs **migration 014** (`014_realtime_orders.sql`) applied to add both tables to the `supabase_realtime` publication. Agent gets the same once Jay's panel lands. UI verified in preview; live fire needs 014 + a real order with a session open. Admin bell lives in the header row (next to name/logout).
...
## Known issues / TODO
- **Try-window EXPIRY not auto-handled** — `finalize_order_if_decided` (migration 015) completes an order + closes the try session once every item is decided (keep/return), called from the customer keep/return actions, so the countdown no longer runs after the customer decides everything. But if the window *expires* with items still `pending` (rider leaves), nothing auto-resolves them yet — needs an expiry job (auto-return undecided + complete). Agent panel / a scheduled task.
