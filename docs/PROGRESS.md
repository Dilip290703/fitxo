...
- [~] **Store new-order alerts** — D — `OrderAlertsProvider` now subscribes to the manager's own `notifications` (kind=`new_store_order`), created by the trigger in **migration 022** (fires per order line, deduped per order/manager). Replaced the old `order_items` INSERT subscription, which silently stopped delivering because its routing policy is a `products` join Realtime can't evaluate. Then `loadStoreOrder` fills the summary. **Built — live fire needs 022.**
