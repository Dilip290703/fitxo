...
- **⚠️ Delivery fee still ₹0 (deferred — #1 in the 2026-06-28 flaw review).** Checkout hardcodes `delivery_fee: 0` despite `system_settings.delivery_fee`/`free_delivery_above`, so **riders earn ₹0** and agent payouts compute 0. Wire the fee into checkout + decide rider pay (full fee vs configurable cut) before real use.
