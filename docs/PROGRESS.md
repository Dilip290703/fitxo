...
- [~] **Live new-order alerts (Store + Admin)** — Supabase Realtime pop-up + bell + chime when a new order is placed; no polling. Store subscribes to `order_items` INSERT (RLS-scoped, deduped per order), Admin to `orders` INSERT. Needs **migration 018** (`018_realtime_orders.sql`) applied to add both tables to the `supabase_realtime` publication. UI verified in preview; live fire needs 018 + a real order with a session open. Admin bell lives in the header row (next to name/logout).
- [~] **Live rider alerts (Agent)** — D, 2026-06-27 — `JobAlertsProvider` (mounted in `AgentShell`, verified-riders only) brings the same pop-up + bell + chime to the agent panel, with three event types: **new job**, **try-on started** (`try_sessions` → status='active'), **item decided** (`order_items` UPDATE → keep/return). Pop-ups deep-link to `/deliveries/[id]`. ⚠️ **The "new job" alert does NOT listen to the `deliveries` assign-UPDATE** — Supabase `postgres_changes` doesn't reliably deliver an UPDATE that moves a row *into* a user's RLS visibility (`rider_id` NULL→rider), so the rider got nothing ("alert only went to admin"). Fix (**migration 021**): a trigger inserts a **born-visible** `notifications` row (owned by the rider, `data.kind='new_job'`) on assignment, and the agent subscribes to `notifications` INSERT filtered to its `user_id` — fires every time. The try-on/decision events listen on already-visible rows (the delivery is assigned), so those are fine. Needs **migration 021** (trigger + `notifications` on the realtime publication). **Built — live fire needs 021 + a real assigned delivery with the agent app open.**
...
## Known issues / TODO
- **Migrations 020 + 021 not yet applied** — **020** (`020_agent_payouts_alerts.sql`) creates `agent_payouts` (+ RLS) for Admin Agent Payouts (and sets `REPLICA IDENTITY FULL` on `deliveries`, now only belt-and-braces). **021** (`021_agent_job_notifications.sql`) adds the assign→notification trigger + puts `notifications` on the realtime publication — **required for the agent "new job" pop-up to fire at all.** Both idempotent. Apply before testing #18 / the agent alerts.
