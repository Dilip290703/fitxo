...
- [~] **Live rider alerts (Agent)** — D, 2026-06-27 — `JobAlertsProvider` (mounted in `AgentShell`, verified-riders only) brings the same pop-up + bell + chime to the agent panel, with three event types, all routed for free by RLS (migration 014) so a rider only ever sees their own: **new job** (`deliveries` UPDATE → status='assigned'), **try-on started** (`try_sessions` → status='active'), and **item decided** (`order_items` UPDATE → decision keep/return). Pop-ups deep-link to `/deliveries/[id]`. Needs **migration 020** (`REPLICA IDENTITY FULL` on `deliveries` so the assign-UPDATE's RLS transition delivers; `deliveries` was already in the publication via 014). **Built — live fire needs 020 + a real assigned delivery with the agent app open.**
...
## Known issues / TODO
- **Migration 020 not yet applied** (`020_agent_payouts_alerts.sql`) — creates `agent_payouts` (+ RLS) for Admin Agent Payouts and sets `REPLICA IDENTITY FULL` on `deliveries` for the agent "new job" pop-up. Idempotent. Apply before testing #18 / the agent alerts.
